  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        localStorage.setItem('userType', (role === 'admin' || role === 'owner') ? 'owner' : 'traveler');
      ...
      <div ref={formRef} className="relative z-10 w-full max-w-[440px] px-6">
        <div className="bg-white/80 backdrop-blur-xl rounded-[32px] shadow-2xl p-8 md:p-10 border border-white/40">
          ...
          <div className="bg-[#767680]/15 p-1 rounded-xl flex mb-8 relative">
            ...
            <div
              className={`absolute top-1 bottom-1 w-[calc(50%-4px)] bg-white rounded-[9px] shadow-sm transition-all duration-300 ease-out ${userType === 'traveler' ? 'left-1' : 'left-[calc(50%+2px)]'
                }`}
            ></div>
            ...
            <button
              ...
              className={`flex-1 relative z-10 py-1.5 text-[13px] font-semibold text-center transition-colors duration-300 ${userType === 'traveler' ? 'text-black' : 'text-gray-500 hover:text-gray-700'
                }`}
            >
              Traveler
            </button>
            <button
              ...
              className={`flex-1 relative z-10 py-1.5 text-[13px] font-semibold text-center transition-colors duration-300 ${userType === 'owner' ? 'text-black' : 'text-gray-500 hover:text-gray-700'
                }`}
            >
              Hotel Owner
            </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-3.5 rounded-2xl font-semibold text-[15px] transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-500/20 ${loading
